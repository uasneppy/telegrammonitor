...
async function showProximityRadiusScreen(ctx) {
  ...
  if (!gpsLocation || !gpsLocation.latitude || !gpsLocation.longitude) {
    await ctx.reply(
      '📏 Радіус попередження\n\n' +
      'У тебе ще не збережена GPS локація.\n\n' +
      '💡 Спочатку поділись своєю локацією в розділі "📍 Моя локація", щоб налаштувати радіус попередження.',
      Markup.keyboard([
        ['📍 Моя локація'],
        ['« Назад']
      ]).resize()
...
ВАЖЛИВО:
- Звіт має бути коротким (максимум 10 рядків)
- Використовуй простий текст БЕЗ заголовків Markdown (##, ###)
- НЕ ДОДАВАЙ заголовки на зразок "## Звіт про загрози"
- НЕ ДОДАВАЙ дату в заголовку
- Просто напиши короткий звіт з емодзі
- Формат: проста структура з емодзі та короткими реченнями`;
